...
class adminPage extends StatefulWidget {
  ...
  @override
  adminPageState createState() => adminPageState(user: user);
}

class adminPageState extends State<adminPage> {
  User user;

  adminPageState({required this.user});
                  ...
                  DropdownButton<String>(
                    items: ['Select Difficulty', 'Easy', 'Medium', 'Hard']
                      ...
                      return DropdownMenuItem<String>(
                        ...
                        child: Text(
                          value,
                          style: TextStyle(color: isDarkMode ? Colors.red : Colors.black),
                        ),
                      );
                    }).toList(),
                  ...
                  if (difficultyValue != 'Select Difficulty') {
                    if (chooseOptionValue == 'Think Tight')
                      Navigator.push(context,
                          MaterialPageRoute(builder: (context) => gamemode2(difficultyLevel: difficultyLevel, user: user)));
                    else if (chooseOptionValue == 'Choose Right')
                      Navigator.push(
                          context, MaterialPageRoute(builder: (context) => FoodGamemode(difficultyValue: difficultyValue, user: user)));
                    else
                      gmError(context);
                  } else {
                    diffError(context);
